...
for (var i in wireTwoPathArray) {
    console.log(wireTwoCoordinates[wireTwoCoordinates.length - 1]);
    var lastPosition = wireTwoCoordinates[wireTwoCoordinates.length - 1].split(",");
    lastPosition = [Number(lastPosition[0]),Number(lastPosition[1])];
    var direction = wireTwoPathArray[i][0];
    var amount = Number(wireTwoPathArray[i].slice(1));
    switch (wireTwoPathArray[i][0]) {
        case "R":
            for (var current = 1; current <= amount; current++) {
                wireTwoCoordinates.push((lastPosition[0] + current) + "," + lastPosition[1]);
            }
            break;
        case "U":
            for (var current = 1; current <= amount; current++) {
                wireTwoCoordinates.push(lastPosition[0] + "," + (lastPosition[1] + current));
            }
            break;
        case "L":
            for (var current = 1; current <= amount; current++) {
...
var matches = [];
var totalStepCountOne = [];
var totalStepCountTwo = [];
for (var i in wireOneCoordinates) {
    if (wireTwoCoordinates.indexOf(wireOneCoordinates[i]) > -1) {
        matches.push(wireOneCoordinates[i]);
        totalStepCountOne.push(i);
        totalStepCountTwo.push(wireTwoCoordinates.indexOf(wireOneCoordinates[i]));
    }
}
...
for (var j in matches) {
    var currentSplit = matches[j].split(',');
    var currentLength = Math.abs(Number(currentSplit[0])) + Math.abs(Number(currentSplit[1]));
    if ((shortest === undefined || currentLength < shortest) && currentLength != "0") {
        shortest = currentLength;
        var currentTotalSteps = Number(totalStepCountOne[j]) + Number(totalStepCountTwo[j]);
        if ((amountOfSteps === undefined || currentTotalSteps < amountOfSteps) && currentLength != "0") {
            amountOfSteps = currentTotalSteps;
        }
    }
}
